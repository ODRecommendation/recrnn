...
class SessionRecommender[T: ClassTag](
  ...
  override def buildModel(): AbstractModule[Tensor[T], Tensor[T], T] = {
    ...
    val rnn = Dense[T](itemCount + 1, activation = "relu").inputs(gruLast)

    if (includeHistory) {
      // mlp part
      val itemTable1 = Embedding[T](itemCount + 1, itemEmbed, inputLength = maxLength)
      val inputMLP: ModuleNode[T] = Input(inputShape = Shape(maxLength))
      var mlp = Dense(mlpHiddenLayers(0)).inputs(itemTable1.inputs(inputMLP))
      for (i <- 1 until mlpHiddenLayers.length) {
        mlp = Dense(mlpHiddenLayers(i), activation = "relu").inputs(mlp)
      }
      val mlpLast = Dense(numClasses, activation = "relu").inputs(mlp)
