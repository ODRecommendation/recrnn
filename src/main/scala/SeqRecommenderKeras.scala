...
/**
  ...
  */
class SeqRecommenderKeras[T: ClassTag](
                                   val itemCount: Int,
                                   val numClasses: Int,
  ...
  override def buildModel(): AbstractModule[Tensor[T], Tensor[T], T] = {
    ...
    val itemTable = Embedding[T](itemCount + 1, itemEmbed, inputLength = maxLength).inputs(inputRNN)
    val gru = GRU[T](200, returnSequences = true).inputs(itemTable)
    val rnnOut = Dense[T](numClasses, activation = "relu").inputs(gru)

    var linear = Dense(mlpHiddenLayers(0), activation = "relu").inputs(inputMLP)
    for (i <- 1 until mlpHiddenLayers.length) {
      linear = Dense(mlpHiddenLayers(i), activation = "relu").inputs(linear)
    }

    val merged = Merge.merge[T](List(linear, rnnOut), "sum")
    Model(Array(inputMLP, inputRNN), merged).asInstanceOf[AbstractModule[Tensor[T], Tensor[T], T]]
