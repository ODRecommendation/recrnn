...
object Main extends App{
  ...
  val pipeline = new Pipeline().setStages(Array(skuIndexer, ohe))
  val pipelineModel = pipeline.fit(data)

  val data1 = pipelineModel.transform(data)
  ...
  val data2 = data1.groupBy("SESSION_ID")
    .agg(collect_list("vectors").alias("item"))

  data2.printSchema()
  data2.show()

  val skuPadding = Array.fill[Double](inputLayer)(0.0)
  val skuPadding1 = Array.fill[Array[Double]](10)(skuPadding)
  val bcPadding = sc.broadcast(skuPadding1).value

  import spark.implicits._
  val data3 = data2.rdd.map(r => {
    val item = r.getAs[mutable.WrappedArray[SparseVector]]("item").array
    val item1 = bcPadding ++ item
    val item2 = item1.takeRight(11)
    val label = item1.takeRight(1).head
